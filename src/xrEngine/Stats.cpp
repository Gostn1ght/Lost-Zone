...
void CStats::Show() 
{
	...
	if (psDeviceFlags.test(rsStatistic))
	{
		...
		F.OutNext("Engine: %2.8fms", EngineTOTAL.result);
		F.OutNext("uUpdateCL: %2.8fms", UpdateClient.result);
		F.OutNext("uShedule: %2.8fms", Sheduler.result);
		F.OutNext("Memory: %2.8fms", fMem_calls);

		F.OutNext("Render:          %2.4fms", RenderTOTAL.result);
		//F.OutNext("Render Wait:      %2.4fms", RenderDUMP_Wait_S.result);

		F.OutNext("R_Main:          %2.8fms", RenderMain.result);
		F.OutNext("R_Main_Static: %2.8fms", RenderMainVIS_Static.result);
		F.OutNext("R_Main_Dynamic: %2.8fms", RenderMainVIS_Dynamic.result);

		F.OutNext("R_Deteil_Vis %2.8fms", RenderDUMP_DT_VIS.result);
		F.OutNext("R_Deteil_Render %2.8fms", RenderDUMP_DT_Render.result);
		F.OutNext("R_Deteil_Cache %2.8fms", RenderDUMP_DT_Cache.result);

		F.OutNext("R_Main_Sun: % 2.8fms", RenderSun.result);
		F.OutNext("R_Main_Lights: %2.8fms", RenderLights.result);

		F.OutNext("R_Main_Shadow: %2.2fms", Render_shadow_mp_process.result);
		F.OutNext("R_Main_DSG_HUD:%2.2fms", Render_dsgHUD_UI.result);
		F.OutNext("R_Main_DSG_REN:%2.2fms", Render_dsgrender.result);
		F.OutNext("R_Main_POST:    %2.2fms", Render_postprocess.result);
