...
public:
	...
	virtual void Execute(LPCSTR weather_name) {
		if (!g_pGamePersistent || !Level().Server) return;
...
public:
	CCC_ChangeTeam(LPCSTR N) : IConsole_Command(N) { bEmptyArgsHandled = false; };
...
//	CMD4(CCC_Float,			"sv_shedule_scale",		&sv_object_shedule_scale, 1, 1000);
